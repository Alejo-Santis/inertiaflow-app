...
<svelte:head>
  <title>Iniciar sesión — InertiaFlow</title>
</svelte:head>
...
<div class="flex min-h-screen bg-slate-50 font-sans">
  ...
  <div class="relative hidden w-1/2 overflow-hidden bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-800 lg:flex lg:flex-col lg:justify-between p-12">
    ...
    <div class="relative">
      <h2 class="text-4xl font-bold leading-tight text-white">
        Gestiona tus proyectos<br />con claridad total.
      </h2>
      <p class="mt-4 text-lg text-indigo-200">
        Organiza equipos, asigna tareas y sigue el progreso en tiempo real.
      </p>

      <div class="mt-10 grid grid-cols-3 gap-4">
        {#each [
          { value: '2k+', label: 'Proyectos activos' },
          { value: '98%', label: 'Satisfacción' },
          { value: '15k', label: 'Tareas completadas' },
        ] as stat}
          <div class="rounded-2xl bg-white/10 p-4 backdrop-blur-sm">
            <p class="text-2xl font-bold text-white">{stat.value}</p>
            <p class="mt-0.5 text-sm text-indigo-200">{stat.label}</p>
          </div>
        {/each}
      </div>
    </div>
  ...
  <div class="flex flex-1 flex-col items-center justify-center px-6 py-12">
    <div class="w-full max-w-md">
      ...
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">Bienvenido de vuelta</h1>
        <p class="mt-1.5 text-sm text-slate-500">Ingresa tus credenciales para continuar</p>
      </div>

      <form onsubmit={(e) => { e.preventDefault(); submit(); }} class="mt-8 space-y-5">
        ...
        <div>
          ...
          <div class="relative mt-1.5">
            ...
            <input
              ...
              class="block w-full rounded-xl border py-2.5 pl-10 pr-10 text-sm text-slate-900 placeholder-slate-400 shadow-sm transition
        ...
        <div class="flex items-center justify-between">
          ...
          <Link href={route('password.request')} class="text-sm font-medium text-indigo-600 hover:text-indigo-700">
