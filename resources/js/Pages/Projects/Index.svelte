<script lang="ts">
  ...
  async function confirmDelete(project: any) {
    const result = await Swal.fire({
      ...
      text: `Se eliminará "${project.name}" y todas sus tareas.`,
      icon: 'warning',
                ...
                <div class="flex items-center gap-1.5">
                  {#if project.can_delete}
                    <button
                      ...
                      class="inline-flex items-center justify-center rounded-lg p-1.5 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600"
                      title="Eliminar proyecto"
                    >
