<script lang="ts">
  ...
  let { organizations = [] }: { organizations?: { id: number; uuid: string; name: string; color: string; departments: { id: number; uuid: string; name: string }[] }[] } = $props();
    ...
    <form onsubmit={(e) => { e.preventDefault(); submit(e); }} class="space-y-6">
      ...
      {#if organizations.length > 0}
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label for="organization_id" class="block text-sm font-medium text-slate-700">Organización</label>
              <select
                id="organization_id"
            ...
            <div>
              <label for="department_id" class="block text-sm font-medium text-slate-700">Departamento</label>
              <select
                id="department_id"
