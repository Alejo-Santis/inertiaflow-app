<script lang="ts">
  ...
  let { project, members, available, canManageMembers = false, canDelete = false }: { project: any; members: any[]; available: any[]; canManageMembers?: boolean; canDelete?: boolean } = $props();

  async function confirmDeleteProject() {
    const result = await Swal.fire({
      title: '¿Eliminar proyecto?',
      text: 'Se eliminarán el proyecto y todas sus tareas. Esta acción no se puede deshacer.',
      icon: 'warning',
    ...
    <div class="space-y-6 lg:col-span-2">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        <button
          ...
          class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-rose-200 hover:shadow-md text-left"
        >
          ...
          <div>
            <p class="font-semibold text-rose-700">Eliminar proyecto</p>
            <p class="text-xs text-slate-500">Elimina el proyecto y sus tareas</p>
          </div>
        </button>
        {/if}
      </div>
